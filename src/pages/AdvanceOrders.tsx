  ...
  return <div className="space-y-5">
    ...
    <>
      ...
      <div className="space-y-3 md:hidden">
        {loading ? (
          <p className="text-center py-10 text-sm font-bold text-[#6B7280] bg-white rounded-2xl border border-[#ECE9E2]">Loading advance orders...</p>
        ) : filtered.length === 0 ? (
          <p className="text-center py-10 text-sm font-bold text-[#6B7280] bg-white rounded-2xl border border-[#ECE9E2]">No advance orders match these filters.</p>
        ) : filtered.map(order => (
          <div key={order.id} className="rounded-2xl border border-[#ECE9E2] bg-white shadow-sm p-3.5">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="font-black text-violet-700 text-sm">{order.deposit_id}</p>
                <p className="text-[11px] text-[#8B9389]">{new Date(order.created_at).toLocaleDateString('en-MY')} · {new Date(order.created_at).toLocaleTimeString('en-MY',{hour:'2-digit',minute:'2-digit'})}</p>
              </div>
              <select value={order.status} onChange={e => void changeStatus(order, e.target.value as AdvanceStatus)}
                className={`shrink-0 rounded-lg border px-2 py-1.5 text-[10px] font-black ${STATUS_STYLES[order.status]}`}>
            ...
            <div className="mt-2.5 pt-2.5 border-t border-[#F0EEE9]">
              <p className="font-bold text-sm text-[#273126] break-words">{order.product_name}</p>
            ...
            <div className="mt-2.5 grid grid-cols-2 gap-2 text-xs">
              <div>
                ...
                <p className="text-[#727970]">{order.phone ? formatPhoneDisplay(order.phone) : ''}</p>
              </div>
              <div className="flex items-center gap-1.5 justify-end text-[#727970]">
                <CalendarDays size={13}/>{new Date(`${order.expected_delivery_date}T00:00:00`).toLocaleDateString('en-MY')}
              </div>
            </div>

            <div className="mt-2.5 flex items-center justify-between text-xs bg-[#FAFAF8] rounded-xl px-3 py-2">
              <span>Total <b className="text-[#273126]">{formatCurrency(order.total_amount)}</b></span>
              <span className="text-violet-700">Paid <b>{formatCurrency(order.deposit_amount)}</b></span>
              <span className="text-red-600">Bal <b>{formatCurrency(order.remaining_balance)}</b></span>
            </div>

            <div className="mt-3 flex flex-wrap items-center gap-1.5">
              <button onClick={() => void openDetails(order)} className="h-9 w-9 flex items-center justify-center rounded-lg bg-[#F4F2F6] text-violet-700" title="Details"><Eye size={16}/></button>
              {order.status !== 'completed' && order.status !== 'cancelled' && <button onClick={() => setPaymentOrder(order)} className="h-9 flex-1 min-w-[110px] rounded-lg bg-emerald-600 px-2.5 text-[11px] font-black text-white">Receive Balance</button>}
              {order.status !== 'completed' && <>
                <button onClick={() => printAdvanceReceipt(order)} className="h-9 w-9 flex items-center justify-center rounded-lg bg-amber-50 text-amber-700" title="Print advance receipt"><Printer size={16}/></button>
                <button onClick={() => downloadFile(advanceReceiptPdf(order))} className="h-9 w-9 flex items-center justify-center rounded-lg bg-amber-50 text-amber-700" title="Download advance receipt"><Download size={16}/></button>
                <button onClick={() => whatsappDepositReceipt(order)} className="h-9 w-9 flex items-center justify-center rounded-lg bg-emerald-50 text-emerald-700" title="WhatsApp advance receipt"><MessageCircle size={16}/></button>
              </>}
              {order.status === 'completed' && <>
                <button onClick={() => downloadFile(invoiceFile(order))} className="h-9 w-9 flex items-center justify-center rounded-lg bg-violet-50 text-violet-700" title="PDF invoice"><Download size={16}/></button>
                <button onClick={() => printFinal(order)} className="h-9 w-9 flex items-center justify-center rounded-lg bg-violet-50 text-violet-700" title="Print receipt"><Printer size={16}/></button>
                <button onClick={() => whatsappInvoice(order)} className="h-9 w-9 flex items-center justify-center rounded-lg bg-emerald-50 text-emerald-700" title="WhatsApp invoice"><MessageCircle size={16}/></button>
              </>}
              <button onClick={() => void removeOrder(order)} className="h-9 w-9 flex items-center justify-center rounded-lg bg-red-50 text-red-600" title="Delete order"><Trash2 size={16}/></button>
            </div>
          </div>
        ))}
      </div>
