...
export default function Profile() {
  ...
  const handleSaveProfile = async () => {
    ...
    if (trimPhone && !isValidPhone(trimPhone)) { setSaveErr('Enter a valid Indian mobile number (e.g. 9876543210 or +91 98765 43210).'); return }
    if (!user) return
                ...
                {editing && (
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-textMuted uppercase tracking-wide mb-1.5">
                        Mobile Number
                        <span className="ml-1 font-normal normal-case text-[10px] text-gray-400">Indian mobile (+91)</span>
                      </label>
                              ...
                              <div className="mt-3 space-y-1">
                                {(o.items || []).map((item, i: number) => (
                                  <p key={`meta-${i}`} className="text-[11px] text-textMuted">
                                    {item.name}: {item.unit} • {formatCurrency(item.base_price)}
                                  </p>
                                ))}
                              </div>
